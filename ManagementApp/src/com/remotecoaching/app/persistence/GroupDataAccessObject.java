...
public class GroupDataAccessObject implements DataAccessObjectGenericInterface<Group, Integer> {
	...
	public void create(Group newInstance) {
		...
		String groupsInsertString = "INSERT INTO groups"
				+ "(name) VALUES"
				+ "(?)";
		String groupRolesInsertString = "INSERT INTO group_role "
				+ "(group_id, role_id) "
				+ "SELECT id, ? "
				+ "FROM groups "
				+ "WHERE name=?";
...
//			}
	...
	private void updateGroupRoles(Group updatedInstance) {
		...
		String groupRolesQuery = "SELECT roles.id FROM roles "
				+ "INNER JOIN "
				+ "group_role WHERE "
				+ "group_role.group_id =? AND "
				+ "group_role.role_id=roles.id";
